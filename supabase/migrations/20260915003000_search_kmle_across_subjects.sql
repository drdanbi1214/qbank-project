...
-- 레옵스에서 KMLE 문제를 넣을 때 현재 과목은 검색 순위에만 작은 가점을 준다.
-- 후보 자체는 사용자가 볼 수 있는 모든 과목에서 만들므로, 검색 내용과 더 잘
-- 맞는 다른 과목 문제도 결과에 남고 충분히 강한 일치라면 먼저 표시된다.
...
as $$
  ...
  order by
    r.final_score
      + case
          when p_subject_id is not null and r.subject_id = p_subject_id then 0.15
          else 0
        end desc,
    r.final_score desc,
    r.exam_id,
...
comment on function public.search_kmle_questions(text, uuid, integer) is
  '전체 KMLE 문제를 유사도순으로 검색하되 관련도가 비슷하면 현재 작성 과목을 우선한다.';
