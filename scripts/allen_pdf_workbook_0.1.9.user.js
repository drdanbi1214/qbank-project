...
(function () {
  ...
  function currentProblemData() {
    ...
    const expectedId = idMatch ? Number.parseInt(idMatch[1], 10) : null;
    const candidates = [];
  ...
  function extractAnswer(allLines, choices) {
    ...
    return embeddedAnswers(choices);
  }

  function embeddedAnswers(choices) {
  ...
  function embeddedExplanationHtml() {
    const solution = String(currentProblemData()?.solution || '');
  ...
  function extractBackData(front) {
    ...
    return {
      ...
      explanationHtml: explanation.html || embeddedExplanationHtml(),
