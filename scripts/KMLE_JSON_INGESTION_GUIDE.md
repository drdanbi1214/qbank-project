...
- `scripts/allen_pdf_workbook_0.1.10.user.js`: 현재 Allen 문제/해설 및 원시험 정보 수집기
- `scripts/allen_kmle_json_export_0.10.6.user.js`: 현재 과목 JSON 내보내기 도구
...
원격 복구에 성공하면 등록할 수 있다. 최신 0.10.6은 이미지를 최대 4회 요청하고,
...
0.1.8 이후 문항에는 다음 원시험 정보가 함께 들어간다. 0.1.10은 일반적인 문항 제목뿐
아니라 `202601 | 고득점 대비 핵심 문항`처럼 제목에 시험 위치가 없는 페이지도 현재
URL과 일치하는 Allen 내장 문제 데이터에서 시험·교시·번호를 읽는다. SPA 이동 직후
내장 데이터가 이전 문항을 가리키거나 아직 준비되지 않은 경우에는 현재 문제 URL을
로그인 쿠키와 함께 다시 받아 일치하는 공식 문제 데이터로 보완한다.

- `sourceLabel`: 화면 제목의 전체 표기(예: `임종평23-2 2교시, 43번`)
- `sourceExam`: 시험 이름(예: `임종평23-2`)
- `sourceSession`: 교시 숫자
- `sourceQuestionNumber`: 해당 원시험의 문제 번호

DB에서는 각각 `allen_label`, `allen_exam`, `allen_session`,
`allen_question_number`로 보존한다. 구버전 JSON은 `code`를 시험 이름으로 대신 쓰며,
알 수 없는 교시와 원문제 번호를 추측해서 채우지 않는다.

KMLE 수집 버튼은 PDF 설정과 관계없이 정답과 해설을 항상 연다. 0.1.10은 화면의 정답
색상이 달라진 경우 현재 URL과 일치하는 Allen 내장 문제 데이터의 공식 정답 번호를
보조 판독에 사용한다. 화면 내장 데이터가 없으면 같은 URL의 서버 응답에서 공식 정답을
다시 읽는다. 정답 또는 해설을
판독하지 못한 문항에서는 즉시 멈추므로, 빈 정답으로 과목 전체 JSON이 만들어져서는
안 된다. 일반 `PDF 자동 수집`에만 사용자가 저장한 PDF 설정을 적용한다.

0.1.10과 0.10.6은 내부 PDF 수집 결과를 `allenPdfLastAutoResultV1`에 전달한다. 자동
...
node --check scripts/allen_pdf_workbook_0.1.10.user.js
