...
(function () {
  ...
  const LAST_AUTO_RESULT_KEY = 'allenPdfLastAutoResultV1';
  const ORIGINAL_AUTOMATION_KEY = 'allenPdfKmleAutomationV1';
  const REQUIRED_ORIGINAL_VERSION = '0.1.10';
  ...
  function lastAutoResult() {
    try {
      const value = JSON.parse(sessionStorage.getItem(LAST_AUTO_RESULT_KEY) || 'null');
  ...
  function startOriginalAutoForCrawl(state) {
    ...
    suppressAutoAlerts();
    sessionStorage.removeItem(LAST_AUTO_RESULT_KEY);
    auto.click();
  ...
  async function runCrawlTick() {
    ...
    try {
      ...
      if (state.phase === 'collecting') {
        ...
        const failed = Number(autoResult?.failed || 0) > 0 || /실패\s*[1-9]/.test(progress);
        if (failed) {
          const cause = autoResult?.error || progress || '원인을 기록하지 못했습니다.';
          const failedUrl = autoResult?.pageUrl || location.href;
          ...
          throw new Error([
            `${state.chapter} 수집 중 재시도 후에도 문제가 발생했습니다.`,
            `문항: ${autoResult?.current || state.chapter || '확인 불가'}`,
            `주소: ${failedUrl}`,
            `원인: ${cause}`,
          ].join('\n'));
        }
