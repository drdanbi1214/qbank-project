...
Allen에서 수집한 KMLE JSON을 운영 사이트에 등록하거나 갱신할 때는
[`KMLE_JSON_INGESTION_GUIDE.md`](KMLE_JSON_INGESTION_GUIDE.md)를 따른다.
가이드는 파일 사전 검사, dry-run, 실제 반영, 이미지·표·정답·목차 연결의 사후
자동 대조와 운영 화면 확인까지 완료 조건으로 정한다.
