...
// @version      0.9.0
// @description  Allen 과목 목차를 돌며 모든 대제목을 수집하고 과목별 KMLE JSON 하나로 내보냅니다.
...
(function () {
  ...
  const REQUIRED_ORIGINAL_VERSION = '0.1.6';
  let exporting = false;
  ...
  function clickOriginalAuto() {
    ...
    if (!auto) {
      alert('Allen PDF 수집 스크립트 0.1.6을 함께 켜 주세요.');
      return false;
    }
    auto.click();
    return true;
  ...
  function updateSubjectButton() {
    ...
    if (button) {
      button.textContent = state ? `KMLE 전체 중단 (${Math.min(state.index + 1, state.total)}/${state.total})` : 'KMLE 과목 전체 수집';
    }
    const status = document.getElementById(STATUS_ID);
    if (!status) return;
    if (!state) {
      status.innerHTML = '<b>KMLE 대기</b><br>목차에서 과목 전체 수집을 누르세요.';
      return;
    }
    const labels = {
      toc: '다음 대제목 준비',
  ...
  async function finishSubjectCrawl(state) {
    ...
    if (state.expected && rows.length !== state.expected) {
      throw new Error(`과목 전체 문항 수가 맞지 않습니다. 예상 ${state.expected}개 · 수집 ${rows.length}개`);
    }
    ...
    try {
      ...
      if (state.phase === 'collecting') {
        ...
        if (state.chapterExpected && chapterCount !== state.chapterExpected) {
          throw new Error(`${state.chapter} 문항 수가 맞지 않습니다. 예상 ${state.chapterExpected}개 · 수집 ${chapterCount}개`);
        }
  ...
  async function exportKmle(showAlert = true, sourceRows = null, downloadTitle = '') {
    ...
    try {
      ...
      return { filename, count: items.length, failures: failures.length };
    } finally {
      exporting = false;
