...
// @version      0.1.6
// @description  Allen 문제를 문제집/해설지 분리형 PDF 문서로 모읍니다.
...
(function () {
  ...
  let autoStats = { saved: 0, skipped: 0, failed: 0, current: '' };
  ...
  function findExplanationStartIndex(allLines) {
    const direct = ['CC', '해설'].map((heading) => allLines.indexOf(heading)).filter((index) => index >= 0);
    if (direct.length) return Math.min(...direct);
    const structured = allLines.findIndex((line) => /^(?:\d+\.\s*)?(?:한줄요약|출제\s*Point)$/.test(line));
  ...
  function findExplanationStartElement() {
    for (const text of ['CC', '해설', '1. 한줄요약', '한줄요약', '2. 출제 Point', '출제 Point']) {
  ...
  function extractExplanation(allLines) {
    ...
    const source = allLines.slice(start, end).filter((line, index, array) => line && (index === 0 || line !== array[index - 1]));
    const sections = [];
    let section = null;
    ...
    return sections
      ...
      .map((item) => {
        ...
        const content = item.content.map((line) => `<p>${escapeHtml(line)}</p>`).join('');
        return `${heading}${content}`;
      })
      .join('');
  }

  function explanationAssetsHtml() {
    const startEl = findExplanationStartElement();
    const endEl = findTextElement('커뮤니티 Q&A');
    if (!startEl) return '';
    const top = startEl.getBoundingClientRect().top;
    const bottom = endEl ? endEl.getBoundingClientRect().top : Number.POSITIVE_INFINITY;
    const items = [];

    for (const table of document.querySelectorAll('table')) {
      if (!visible(table)) continue;
      const box = table.getBoundingClientRect();
      if (box.top < top || box.bottom > bottom) continue;
      items.push({ top: box.top, html: cloneTable(table) });
    }
    for (const img of document.images) {
      if (!visible(img) || img.closest('table')) continue;
      const box = img.getBoundingClientRect();
      if (box.top < top || box.bottom > bottom || img.naturalWidth < 80 || img.naturalHeight < 80) continue;
      const src = new URL(img.currentSrc || img.src, location.href).href;
      items.push({ top: box.top, html: `<div class="image"><img src="${escapeHtml(src)}"></div>` });
    }
    if (!items.length) return '';
    return `<h4>해설 이미지·표</h4>${items.sort((a, b) => a.top - b.top).map((item) => item.html).join('')}`;
  }

  function extractAnswer(allLines, choices) {
  ...
  function extractChoiceRates(allLines, choices) {
  ...
  function extractBackData(front) {
    const allLines = lines();
    return {
      answers: extractAnswer(allLines, front.choices),
      choiceRates: extractChoiceRates(allLines, front.choices),
      explanationHtml: extractExplanation(allLines),
      explanationAssetsHtml: explanationAssetsHtml(),
    };
  }
...
<body>
  ...
  function updateProgress(message = '') {
    ...
    panel.innerHTML = `<b>${autoRunning ? 'PDF 자동 수집 중' : 'PDF 수집 대기'}</b> · ${pagePosition()} · 총 ${total} · 저장 ${autoStats.saved} · 건너뜀 ${autoStats.skipped} · 실패 ${autoStats.failed}${autoStats.current ? `<br>현재: ${escapeHtml(autoStats.current)}` : ''}${message ? `<br>${escapeHtml(message)}` : ''}`;
  }
  ...
  async function runAuto() {
    ...
    autoRunning = true;
    autoSession = session;
    autoStats = { saved: 0, skipped: 0, failed: 0, current: '' };
    updateAutoButton();
